# cleanlist_ai.aio.PublicSmartAgentsApi
...
async with cleanlist_ai.aio.ApiClient(configuration) as api_client:
    ...
    api_instance = cleanlist_ai.aio.PublicSmartAgentsApi(api_client)
    smart_agent_task_id = 'smart_agent_task_id_example' # str | Canonical id from /smart-agents/run's `smart_agent_task_id` field

    try:
        # Get smart agent results
        api_response = await api_instance.get_smart_agent_results(smart_agent_task_id)
...
async with cleanlist_ai.aio.ApiClient(configuration) as api_client:
    ...
    try:
        # List smart agent runs
        api_response = await api_instance.list_smart_agents(list_id=list_id, limit=limit)
...
async with cleanlist_ai.aio.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = cleanlist_ai.aio.PublicSmartAgentsApi(api_client)
    run_smart_agent_request = cleanlist_ai.aio.RunSmartAgentRequest() # RunSmartAgentRequest | 

    try:
        # Run a smart agent
        api_response = await api_instance.run_smart_agent(run_smart_agent_request)
