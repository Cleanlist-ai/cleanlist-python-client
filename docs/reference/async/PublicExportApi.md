# cleanlist_ai.aio.PublicExportApi
...
async with cleanlist_ai.aio.ApiClient(configuration) as api_client:
    ...
    api_instance = cleanlist_ai.aio.PublicExportApi(api_client)
    export_csv_request = cleanlist_ai.aio.ExportCsvRequest() # ExportCsvRequest | 

    try:
        # Export a list to CSV (signed URL)
        api_response = await api_instance.export_csv(export_csv_request)
...
async with cleanlist_ai.aio.ApiClient(configuration) as api_client:
    ...
    try:
        # Export a list as JSON
        api_response = await api_instance.export_json(list_id, limit=limit, cursor=cursor, columns=columns)
